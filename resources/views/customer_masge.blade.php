@include('header')
@include('sider')
<main id="main" class="main">

    <div class="pagetitle">
      <h1>Data Tables</h1>
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="index.php">Home</a></li>
          <li class="breadcrumb-item">Tables</li>
          <li class="breadcrumb-item active">Data</li>
        </ol>
      </nav>
    </div>

    <section class="section">
<div  class="row">
  <div class="col-lg-12">

    <div style=" overflow-y:auto;"class="card">
      <div class="card-body">
      <div class="modal fade" id="editmodal" tabindex="-1" aria-labelledby="addServiceModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addServiceModalLabel">Add Menu</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            
    <div class="modal-body">
        <form action="" method="post" enctype="multipart/form-data" id="updateForm">  
            @csrf
            <input type="hidden" id="edit_id"> 
            <div class="mb-3">
                <label for="icon-name" class="form-label">Name</label>
                <input type="text" class="form-control" id="name" placeholder="Enter Your Name" name="name" >
            </div>
            <div class="mb-3">
                <label for="icon-name" class="form-label">L_name</label>
                <input type="text" class="form-control" id="l_name" placeholder="Enter Your lname" name="l_name" >
            </div>
            <div class="mb-3">
                <label for="icon-name" class="form-label">Email</label>
                <input type="text" class="form-control" id="email" placeholder="Enter Your Email" name="email" >
            </div>
            <div class="mb-3">
                <label for="icon-name" class="form-label">Messge</label>
                <input type="text" class="form-control" id="messge" placeholder="Enter Your Messge" name="messge" >
            </div>   
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
            <button type="submit" class="btn btn-primary" name="submit">UPdate Data</button>
        </form>
    </div>
 </div>
</div>
</div>
<table class="table datatable">
        <thead>
          <tr>
            <th>id</th>
            <th>Name</th>
            <th>L Name</th>
            <th>Email</th>
            <th>Message</th>
            <th>Action</th>
          </tr>
        </thead>
        <tbody>
        @foreach ($messge as $items)
        <tr>
            <td>{{ $items->id }}</td>
            <td>{{ $items->name }}</td>
            <td>{{ $items->l_name }}</td>
            <td>{{ $items->email }}</td>
            <td>{{ $items->messge }}</td>
            <!-- <td><img src="{{ asset('storage/' . $items->image_1) }}" alt="Image" width="90" height="70px;"></td>  -->
            <td>

              <button class="btn btn-primary editbtn btn-sm " value="{{ $items->id }}">Update</button>
              <a class="btn btn-danger  btn-sm" href="{{ route('delete.massge', ['id' => $items]) }}" onclick="return confirm('Are you sure?');">Delete</a>
            </td>
        </tr>
       @endforeach
       </tbody>
       </table>
       </div>
  </div>
</div>
</div>
</section>
</main>

<script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>

<script>
  $(document).ready(function () {
    $(document).on('click', '.editbtn', function () {
        var id = $(this).val();
        $.ajax({
            url: '/massgeedit/' + id,
            type: 'GET',
            success: function (response) {
                if (response.data) {
                    $('#edit_id').val(response.data.id);
                    $('#name').val(response.data.name);
                    $('#l_name').val(response.data.l_name);
                    $('#email').val(response.data.email);
                    $('#messge').val(response.data.messge);
                    $('#editmodal').modal('show'); 
                 }
            },
            error: function () {
                alert('Error loading data');
            }
        });
    });

    
    $('#updateForm').on('submit', function (e) {
        e.preventDefault();
        var id = $('#edit_id').val();
        var formData = new FormData(this);
        formData.append('_method', 'PUT'); 

        $.ajax({
            url: '/edit-massge/' + id,
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
            success: function (response) {
                if (response.success) {
                    location.reload(); 
                } else {
                    alert('Error updating data');
                }
            },
            error: function () {
                alert('Error in updating');
            }
        });
    });
});
</script>

       @include('footer')